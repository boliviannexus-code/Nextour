...
@section('page-subtitle', 'Empresas registradas, aprobadas o creadas desde autoregistro')
            ...
            <tbody>
                @forelse ($companies as $company)
                    <tr>
                        ...
                        <td>
                            <div>{{ trim(($company->city ?: '').', '.($company->country ?: ''), ', ') ?: '-' }}</div>
                            @if ($company->website)
                                <a class="small" href="{{ $company->website }}" target="_blank" rel="noopener">Sitio web</a>
                            @endif
                        </td>
                        ...
                        <td class="text-end">
                            ...
                            @can('companies.update')
                                @if ($company->registrationRequest?->status !== \App\Models\RegistrationRequest::STATUS_REJECTED)
                ...
                @empty
                    <x-ui.empty-row colspan="8" message="No hay empresas registradas." />
                @endforelse
            </tbody>
        </table>
